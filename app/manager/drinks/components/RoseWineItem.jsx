...
export default function RoseWineItem(props){
    ...
    function editWine(id,
        ...
        document.querySelector('#form h1').textContent = 'EDIT ROSÉ'
        document.querySelector('#form').style.background = 'lightblue'
        document.querySelector('#submit-button-form').innerHTML = `Update Rosé`
        setEditMode(true)
                ...
                >
                  ...
                  <div className="typos" style={{color:'red'}} dangerouslySetInnerHTML={{__html:props.typos}}></div>
                    ...
                    <span   className="item-button delete-button"
                            // onClick={()=>deleteMenuItem(props.id,'drink','rose','/manager/drinks')}
                            onClick={()=>{
                              if(confirm(`
Are you sure you want to permanently delete this menu item:
  ...
  ${props.name2}                                
                                `)){
                                deleteMenuItem(props.id,'drink','rose','/manager/drinks')
                              }else{
                                return
                              }
                            }}                                                        
                    ...
                    <span onClick={()=>moveUp(props.id,'drink','rose','/manager/drinks')}
                          className={`item-button arrow-button ${props.sequence == 1 ? 'visibility-hidden' : ''}`}>
                        <ImArrowUp style={{ position:'relative',
                                            
                                            top:'1px'}} />
                    </span>
                    <span onClick={()=>moveDown(props.id,'drink','rose','/manager/drinks')}
