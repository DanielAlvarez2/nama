...
export default function BeerItem(props){
    ...
    function editBeer(id,
        ...
        document.querySelector('#form h1').textContent = 'EDIT BEER'
        document.querySelector('#form').style.background = 'lightblue'
        document.querySelector('#submit-button-form').innerHTML = `Update Beer`
        setEditMode(true)
                ...
                >
                  ...
                  <div className="typos" style={{color:'red'}} dangerouslySetInnerHTML={{__html:props.typos}}></div>
                    ...
                    <span   className="item-button delete-button"
                            // onClick={()=>deleteMenuItem(props.id,'drink','beer','/manager/drinks')}
                            onClick={()=>{
                              if(confirm(`
Are you sure you want to permanently delete this menu item:
 ...
 - ${props.name1}                                
                                `)){
                                deleteMenuItem(props.id,'drink','beer','/manager/drinks')
                              }else{
                                return
                              }
                            }}                                                        
                    ...
                    <span onClick={()=>moveUp(props.id,'drink','beer','/manager/drinks')}
                          className={`item-button arrow-button ${props.sequence == 1 ? 'visibility-hidden' : ''}`}>
                        <ImArrowUp style={{ position:'relative',
                                            
                                            top:'1px'}} />
                    </span>
                    <span onClick={()=>moveDown(props.id,'drink','beer','/manager/drinks')}
