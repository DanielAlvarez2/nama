...
export default function WhiteWineItem(props){
    ...
    function editWine(id,
        ...
        document.querySelector('#form h1').textContent = 'EDIT WHITE WINE'
        document.querySelector('#form').style.background = 'lightblue'
        document.querySelector('#submit-button-form').innerHTML = `Update White Wine`
        setEditMode(true)
...
Are you sure you want to permanently delete this menu item:
  ...
  ${props.name2}                                
                                `)){
                                deleteMenuItem(props.id,'drink','white wine','/manager/drinks')
                              }else{
                                return
                              }
                            }}                                                        
                    ...
                    <span onClick={()=>moveUp(props.id,'drink','white wine','/manager/drinks')}
                          className={`item-button arrow-button ${props.sequence == 1 ? 'visibility-hidden' : ''}`}>
                        <ImArrowUp style={{ position:'relative',
                                            
                                            top:'1px'}} />
                    </span>
                    <span onClick={()=>moveDown(props.id,'drink','white wine','/manager/drinks')}
